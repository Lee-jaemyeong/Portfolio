...
**🔹트러블슈팅**
  **1. 카카오 로그아웃 실패**
     - 문제 상황: 카카오 로그아웃 시 세션 쿠키가 정상적으로 삭제되지 않아 로그인 상태가 유지되는 현상이 발생 → 보안 취약점 존재
     - 원인 분석: 카카오 로그아웃 API 호출 또는 로그아웃 URL 리다이렉션이 누락 → 카카오 측 세션 및 인증 토큰이 미해제
     - 해결 방법: Spring Security 설정에 카카오 로그아웃 URL을 명시 → 로그아웃 요청 시 해당URL로 리다이렉트되도록 구현 → 카카오 세션을 종료 → 클라이언트 쿠키 삭제 처리
  **2. 유저 연결테이블에 따른 탈퇴실패**
     - 문제 상황: 데이터베이스 제약조건으로 인해 사용자 삭제 실패
     - 원인 분석: 사용자 엔티티와 연관된 다른 엔티티 간의 외래키 관계 존재 → 연관 데이터가 삭제 또는 해제되지 않아 제약 조건을 위반
     - 해결 방법: JPA 엔티티에 cascade = CascadeType.REMOVE 설정 적용 → 사용자 삭제 시 연관된 엔티티도 삭제 처리
  **3. 그룹장인 유저 탈퇴실패**
     - 문제 상황: 데이터베이스 제약조건으로 인해 사용자 삭제 실패
     - 원인 분석: 사용자 엔티티와 연관된 다른 엔티티 간의 외래키 관계 존재 → 연관 데이터가 삭제 또는 해제되지 않아 제약 조건을 위반
     - 해결 방법: JPA 엔티티에 cascade = CascadeType.REMOVE 설정 적용 → 사용자 삭제 시 연관된 엔티티도 삭제 처리
  **4. Git 병합오류**
     - 문제 상황:  다수 개발자가 동시에 작업하여 병합 충돌이 발생
     - 원인 분석: 세밀하지 않은 파트 분배 및 중복된 파일로 여러 사람이 개발 → 충돌문제 발생
     - 해결 방법: 팀원들에게 개발파일을 받아 수동으로 병합후 Git main브랜치에 Push하는 방식으로 해결
...
**🔹트러블슈팅**
  **1. 글과 댓글 즉시 반영**
     - 문제 상황: 글과 댓글 작성시 페이지를 수동 리다이렉트해야 반영 → 유저 편의성 문제
     - 원인 분석: useEffect, useCallback 등의 최적화 미흡
     - 해결 방법: useCallback과 useEffect를 사용 → 리렌더링 최소화 → 최신 데이터가 UI에 즉시 반영
  **2. 글 목록 무한 스크롤 시 중복 데이터 로딩**
     - 문제 상황: 글 리스트에서 데이터 중복 오류 발생
     - 원인 분석: 글 리스트를 불러올 때 기준값 누락
     - 해결 방법: 글 리스트를 불러올 때 lastId를 기준으로 불러오도록 기준값 부여
  **3. 지도 링크가 메인페이지에서 보이지 않는 오류**
     - 문제 상황: 지도 링크를 메인 페이지에서 불러올 수 없는 오류 발생
     - 원인 분석: 로컬스토리지에 저장된 링크를 사용하는 로직 누락
     - 해결 방법: localStorage.getItem(‘kakaoMapLink’) 로직 부여 → setLocationLink로 상태값 연동해 사용
  **4. 비공개 상태인 리트윗 글 표시**
     - 문제 상황: 비공개 상태인 리트윗된 글이 리스트에 표시되는 문제 발생
     - 원인 분석: 리트윗된 글의 공개범위 상태 확인 구문 누락
     - 해결 방법: 리트윗한 글의 상태를 확인하는 공개범위 검사 조건 추가
  **5. 비로그인 유저 좋아요 클릭**
     - 문제 상황: 비로그인 상태의 유저가 좋아요를 클릭 가능한 문제 발생
     - 원인 분석: 좋아요를 누르는 조건에 Userid검사가 누락되어 발생 문제
     - 해결 방법: if문을 사용해 !Userid라면 좋아요를 누를 시 로그인 오류 처리    
**🔹협업 및 소감**
...
**🔹트러블슈팅**
  **1. 예약 날짜 일괄 비활성화 오류**
     - 문제 상황: 단일 장소에서 예약한 날짜가 전체 예약 건에 일괄 적용 → 모든 장소의 동일한 날짜가 예약 불가능 처리
     - 원인 분석: 장소(place_id) 조건 없이 날짜만을 기준으로 비활성화 → 모든 장소에 동일한 날짜 비활성화 적용
     - 해결 방법: 예약 비활성화 로직에서 장소 식별자(place_id) 조건 포함 → JOIN 구문을 통해 장소별 예약 날짜 필터링 → 장소별로 독립된 비활성화 날짜 처리 가능
  **2. 결제 시 예약 정보 누락**
     - 문제 상황: 결제 진행 시, 예약 정보가 화면에 미출력 혹은 null 값으로 표시되는 문제 발생
     - 원인 분석: 결제 매퍼에서 예약id만 조회하고, 이에 따른 예약 상세 정보를 Join하여 가져오지 않아 필요한 정보가 누락
     - 해결 방법: 결제 매퍼에 예약 테이블과의 JOIN 구문을 추가 → 예약 ID 뿐만 아니라 관련된 예약 상세 정보를 함께 조회하도록 수정
  **3. 결제 취소 및 환불 시 예약 상태 미반영**
     - 문제 상황: 결제 취소 또는 환불을 진행해도 해당 예약 상태가 여전히 "예약 중"으로 유지되는 문제 발생
     - 원인 분석: 결제 및 예약 상태 간 연결 관계 미설정 → 결제 취소 처리 시 예약 상태 업데이트 실패
     - 해결 방법: 예약 상태와 결제 상태를 Enum 타입으로 정의 → 예약 상태가 취소일 경우에만 결제 취소 또는 환불을 가능하게 하는 로직 구성 → 결제 상태 변경과 예약 상태 변경을 동기화
  **4. 일괄 예약 시 경로 정보 미조회**
     - 문제 상황: 여러 장소를 포함하는 일괄 예약 시, 경로 정보 조회 실패 → 예약 데이터가 null로 저장되는 오류 발생
     - 원인 분석: 초기 테스트에서는 더미 데이터를 기반으로 경로 ID만 연결하여 기능 확인 → 실제 경로 정보 테이블과의 관계 설정이 누락
     - 해결 방법: 예약 테이블이 경로 정보 테이블과 관계를 맺도록 수정 → JOIN 구문을 통해 일괄 예약 시 각 경로의 정보를 조회할 수 있도록 개선
  **5. 일괄결제 취소, 환불 오류**
     - 문제 상황: 일괄 결제의 취소 또는 환불 요청 미처리
     - 원인 분석: 일괄 결제는 여러 예약을 경로 ID 기준으로 묶어 총 금액을 계산하는 구조 → 결제 테이블에 예약 ID 저장 불가 → 환불 처리 시 참조할 예약 정보가 미존재
     - 해결 방법: 결제 테이블 중심이 아닌, 각 예약이 결제 ID를 참조하도록 구조 개선 → 환불 로직에서는 동일 결제id를 가지는 예약들을 묶어 처리하도록 변경 → 일괄 결제에 대한 환불도 정상 작동하게 개선
**🔹협업 및 소감**
